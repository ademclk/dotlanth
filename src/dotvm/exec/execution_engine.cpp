...
ExecutionEngine::ExecutionEngine(core::VmContext& ctx) noexcept
    : vm_ctx_{ctx}, exec_ctx_{}, debug_ctx_{}, const_pool_{} {}
...
bool ExecutionEngine::execute_instruction(std::uint32_t instr) noexcept {
    ...
    switch (opcode_val) {
        ...
        case opcode::RET: {
            ...
            const auto end_reg =
                static_cast<unsigned>(frame.base_reg) + static_cast<unsigned>(frame.local_count);
        ...
        case opcode::TRY: {
            ...
            auto handler_pc = static_cast<std::size_t>(static_cast<std::int64_t>(exec_ctx_.pc - 1) +
                                                       handler_offset);

            // Create and push exception frame
            auto frame = core::ExceptionFrame::make(handler_pc,
                                                    exec_ctx_.pc - 1,  // TRY instruction location
                                                    vm_ctx_.call_stack().depth(), catch_types);
        ...
        case opcode::LEA: {
            ...
            auto base_addr =
                static_cast<std::int64_t>(reinterpret_cast<std::uintptr_t>(*ptr_result));
...
ExecResult ExecutionEngine::dispatch_loop() noexcept {
    ...
    __label__
        ...
        op_ADD,
        op_SUB, op_MUL, op_DIV, op_MOD, op_NEG, op_ABS, op_ADDI, op_SUBI, op_MULI,
        // Bitwise (0x20-0x2F)
        op_AND, op_OR, op_XOR, op_NOT, op_SHL, op_SHR, op_SAR, op_ANDI, op_ORI, op_XORI,
        // Comparison (0x30-0x3F) - EXEC-009
        op_EQ, op_NE, op_LT, op_LE, op_GT, op_GE, op_LTU, op_LEU, op_GTU, op_GEU, op_TEST,
        op_CMPI_EQ, op_CMPI_NE, op_CMPI_LT, op_CMPI_GE,
        // Control Flow (0x40-0x5F) - EXEC-005
        op_JMP, op_JZ, op_JNZ, op_BEQ, op_BNE, op_BLT, op_BLE, op_BGT, op_BGE, op_CALL, op_RET,
        op_HALT,
        // Exception Handling (0x52-0x55) - EXEC-011
        op_TRY, op_CATCH, op_THROW, op_ENDTRY,
        // Memory Load/Store (0x60-0x68) - EXEC-006
        op_LOAD8, op_LOAD16, op_LOAD32, op_LOAD64, op_STORE8, op_STORE16, op_STORE32, op_STORE64,
        op_LEA,
        // Data Move (0x80-0x8F)
...
op_TRY: {
    ...
    // Compute handler PC (relative to current PC, which is already advanced)
    auto handler_pc =
        static_cast<std::size_t>(static_cast<std::int64_t>(exec_ctx_.pc - 1) + handler_offset);
...
std::vector<std::uint8_t> ExecutionEngine::inspect_memory(core::Handle handle, std::size_t offset,
...
std::uint32_t ExecutionEngine::jit_register_function(std::size_t entry_pc,
                                                     std::size_t end_pc) noexcept {
...
std::uint64_t ExecutionEngine::jit_register_loop(std::uint32_t func_id, std::size_t header_pc,
