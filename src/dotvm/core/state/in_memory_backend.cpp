...
public:
    ...
    [[nodiscard]] Result<std::vector<std::byte>> get_at_version(
        Key key, std::uint64_t version) const override {
        auto key_vec = std::vector<std::byte>(key.begin(), key.end());
    ...
    [[nodiscard]] Result<void> iterate_at_version(
        Key prefix, std::uint64_t version, const IterateCallback& callback) const override {
        auto prefix_vec = std::vector<std::byte>(prefix.begin(), prefix.end());
