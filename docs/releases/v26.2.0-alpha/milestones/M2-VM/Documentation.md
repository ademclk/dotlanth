...
- Bundle export snapshots the input `.dot` file, writes `trace.jsonl`, records a stable `state_kv` diff in `state_diff.json`, and records declared capabilities plus stable `used`/`denied` accounting in `capability_report.json`.
- DotDB now indexes finalized bundles under `.dotlanth/bundles/<run_id>/` by external `run_id`, storing the bundle ref plus `manifest.json` SHA-256 and byte count.
