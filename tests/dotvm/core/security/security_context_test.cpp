...
TEST(SecurityContextErrorTest, ToStringAllValues) {
    ...
    EXPECT_STREQ(to_string(SecurityContextError::PermissionDenied), "PermissionDenied");
    EXPECT_STREQ(to_string(SecurityContextError::MemoryLimitExceeded), "MemoryLimitExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::AllocationCountExceeded),
                 "AllocationCountExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::AllocationSizeExceeded), "AllocationSizeExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::InstructionLimitExceeded),
                 "InstructionLimitExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::StackDepthExceeded), "StackDepthExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::TimeLimitExceeded), "TimeLimitExceeded");
    EXPECT_STREQ(to_string(SecurityContextError::ContextInvalid), "ContextInvalid");
}
