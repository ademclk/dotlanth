...
TEST(AuditSeverityTest, Ordering) {
    EXPECT_LT(static_cast<int>(AuditSeverity::Debug),
              static_cast<int>(AuditSeverity::Info));
    EXPECT_LT(static_cast<int>(AuditSeverity::Info),
              static_cast<int>(AuditSeverity::Warning));
    EXPECT_LT(static_cast<int>(AuditSeverity::Warning),
              static_cast<int>(AuditSeverity::Error));
    EXPECT_LT(static_cast<int>(AuditSeverity::Error),
              static_cast<int>(AuditSeverity::Critical));
}
...
TEST(AuditEventTypeTest, ToStringAllValues) {
    ...
    EXPECT_STREQ(to_string(AuditEventType::CapabilityCreated),
                 "CapabilityCreated");
    EXPECT_STREQ(to_string(AuditEventType::CapabilityRevoked),
                 "CapabilityRevoked");
    EXPECT_STREQ(to_string(AuditEventType::MemoryLimitExceeded),
                 "MemoryLimitExceeded");
    EXPECT_STREQ(to_string(AuditEventType::SecurityViolation),
                 "SecurityViolation");
}

TEST(AuditEventTypeTest, DefaultSeverity) {
    // Critical severity
    EXPECT_EQ(default_severity(AuditEventType::SecurityViolation),
              AuditSeverity::Critical);

    // Error severity
    EXPECT_EQ(default_severity(AuditEventType::PermissionDenied),
              AuditSeverity::Error);
    EXPECT_EQ(default_severity(AuditEventType::AllocationDenied),
              AuditSeverity::Error);
    EXPECT_EQ(default_severity(AuditEventType::DotFailed),
              AuditSeverity::Error);
    EXPECT_EQ(default_severity(AuditEventType::MemoryLimitExceeded),
              AuditSeverity::Error);

    // Warning severity
    EXPECT_EQ(default_severity(AuditEventType::CapabilityRevoked),
              AuditSeverity::Warning);

    // Info severity
    EXPECT_EQ(default_severity(AuditEventType::DotStarted),
              AuditSeverity::Info);
    EXPECT_EQ(default_severity(AuditEventType::DotCompleted),
              AuditSeverity::Info);
    EXPECT_EQ(default_severity(AuditEventType::ContextCreated),
              AuditSeverity::Info);
}
...
TEST(AuditEventTest, NowFactory) {
    ...
    auto event = AuditEvent::now(AuditEventType::DotStarted,
                                 AuditSeverity::Info, Permission::Execute, 42);
...
TEST(AuditEventTest, ForDotFactory) {
    auto event = AuditEvent::for_dot(123, AuditEventType::DotStarted,
                                     AuditSeverity::Info, "Starting dot 123");
    EXPECT_EQ(event.type, AuditEventType::DotStarted);
...
TEST(AuditEventTest, ForCapabilityFactory) {
    auto event =
        AuditEvent::for_capability(456, AuditEventType::CapabilityCreated,
...
TEST(AuditEventTest, FluentAPIMessage) {
    auto event = AuditEvent::now(AuditEventType::DotStarted)
                     .with_message("Execution started");
...
TEST(SimpleBufferedAuditLoggerTest, QueryBySeverity) {
    ...
    logger.log(AuditEvent::now(AuditEventType::DotStarted,
                               AuditSeverity::Info));
    logger.log(AuditEvent::now(AuditEventType::DotFailed,
                               AuditSeverity::Error));
...
TEST(SimpleBufferedAuditLoggerTest, RingBufferOverwrite) {
    ...
    for (int i = 0; i < 10; ++i) {
        logger.log(
            AuditEvent::now(AuditEventType::AllocationAttempt)
