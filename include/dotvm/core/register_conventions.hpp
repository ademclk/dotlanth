...
[[nodiscard]] constexpr RegisterClass classify_register(std::uint8_t reg) noexcept {
    if (reg == 0) return RegisterClass::Zero;
    if (reg <= 15) return RegisterClass::CallerSaved;
    if (reg <= 31) return RegisterClass::CalleeSaved;
    return RegisterClass::General;
}

[[nodiscard]] constexpr bool is_caller_saved(std::uint8_t reg) noexcept {
    return reg >= reg_range::CALLER_SAVED_START &&
           reg <= reg_range::CALLER_SAVED_END;
}

[[nodiscard]] constexpr bool is_callee_saved(std::uint8_t reg) noexcept {
    return reg >= reg_range::CALLEE_SAVED_START &&
           reg <= reg_range::CALLEE_SAVED_END;
}
