...
public:
    ...
    [[nodiscard]] virtual Result<std::vector<std::byte>> get_at_version(
        Key key, std::uint64_t version) const {
        (void)key;
        (void)version;
    ...
    /// @param callback Called for each matching key-value pair
    /// @return Success, or error code
    [[nodiscard]] virtual Result<void> iterate_at_version(
        Key prefix, std::uint64_t version, const IterateCallback& callback) const {
        (void)prefix;
        (void)version;
        (void)callback;
        return StateBackendError::UnsupportedOperation;
    ...
    virtual void set_min_snapshot_version(std::uint64_t min_version) noexcept {
        (void)min_version;
    }
};
