    ...
    for (std::size_t i = 0; i < Vector<Width, Lane>::kLaneCount; ++i) {
        if constexpr (std::is_floating_point_v<Lane>) {
            if constexpr (std::is_same_v<Lane, float>) {
                ...
                result[i] = ((bits & 0x80000000) != 0) ? a[i] : b[i];
            } else {
                std::uint64_t bits = 0;
                std::memcpy(&bits, &mask[i], sizeof(bits));
                result[i] = ((bits & 0x8000000000000000ULL) != 0) ? a[i] : b[i];
            }
        } else {
...
public:
    ...
    explicit SimdAlu(Architecture arch = Architecture::Arch128,
        ...
        : arch_{arch}, features_{(features != nullptr) ? *features : detect_cpu_features()} {
    ...
    [[nodiscard]] static SimdAlu auto_detect(const CpuFeatures* features = nullptr) noexcept {
        const auto& feat = (features != nullptr) ? *features : detect_cpu_features();
    ...
    [[nodiscard]] std::size_t vector_width_bits() const noexcept { return arch_bit_width(arch_); }
    ...
    [[nodiscard]] Vector128f32 fma_v128f32(const Vector128f32& a, const Vector128f32& b,
                                           const Vector128f32& c) const noexcept {
        return vfma(a, b, c);
    }

    [[nodiscard]] float dot_v128f32(const Vector128f32& a, const Vector128f32& b) const noexcept {
        return vdot(a, b);
    }
    ...
    [[nodiscard]] Vector256f32 fma_v256f32(const Vector256f32& a, const Vector256f32& b,
                                           const Vector256f32& c) const noexcept {
        return vfma(a, b, c);
    }

    [[nodiscard]] float dot_v256f32(const Vector256f32& a, const Vector256f32& b) const noexcept {
        return vdot(a, b);
    }
    ...
    [[nodiscard]] Vector512f32 fma_v512f32(const Vector512f32& a, const Vector512f32& b,
                                           const Vector512f32& c) const noexcept {
        return vfma(a, b, c);
    }

    [[nodiscard]] float dot_v512f32(const Vector512f32& a, const Vector512f32& b) const noexcept {
        return vdot(a, b);
    }
    ...
    void vadd_i32(VectorRegisterFile& regs, std::uint8_t vd, std::uint8_t vs1,
                  std::uint8_t vs2) const noexcept {
        switch (arch_) {
    ...
    void vsub_i32(VectorRegisterFile& regs, std::uint8_t vd, std::uint8_t vs1,
                  std::uint8_t vs2) const noexcept {
        switch (arch_) {
            case Architecture::Arch128: {
                auto a = regs.read_v128i32(vs1);
    ...
    void vmul_i32(VectorRegisterFile& regs, std::uint8_t vd, std::uint8_t vs1,
                  std::uint8_t vs2) const noexcept {
        switch (arch_) {
            case Architecture::Arch128: {
                auto a = regs.read_v128i32(vs1);
...
// 256-bit AVX2 dispatch
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vadd_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vadd_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vadd_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vadd_avx2_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vadd_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vadd_avx2_f64(a, b);
            }
        }
#endif
...
// 512-bit AVX-512 dispatch
        ...
        if constexpr (Width == 512) {
            if (features_.avx512f && features_.avx512bw) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vadd_avx512_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vadd_avx512_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vadd_avx512_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vadd_avx512_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vadd_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vadd_avx512_f64(a, b);
            }
        }
#endif
...
// 128-bit NEON dispatch
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vadd_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vadd_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vadd_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vadd_neon_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vadd_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vadd_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vsub(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vsub_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vsub_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vsub_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vsub_avx2_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vsub_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vsub_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 512) {
            if (features_.avx512f && features_.avx512bw) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vsub_avx512_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vsub_avx512_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vsub_avx512_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vsub_avx512_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vsub_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vsub_avx512_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vsub_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vsub_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vsub_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vsub_neon_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vsub_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vsub_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vmul(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmul_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmul_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmul_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmul_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 512) {
            if (features_.avx512f && features_.avx512bw) {
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmul_avx512_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmul_avx512_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vmul_avx512_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmul_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmul_avx512_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmul_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmul_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmul_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmul_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmul_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vdiv(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256 && std::is_floating_point_v<Lane>) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vdiv_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vdiv_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 512 && std::is_floating_point_v<Lane>) {
            if (features_.avx512f) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vdiv_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vdiv_avx512_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128 && std::is_floating_point_v<Lane>) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vdiv_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vdiv_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Lane dispatch_vdot(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256 && std::is_floating_point_v<Lane>) {
            if (features_.avx2 && features_.fma3) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vdot_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vdot_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128 && std::is_floating_point_v<Lane>) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vdot_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vdot_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vfma(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256 && std::is_floating_point_v<Lane>) {
            if (features_.avx2 && features_.fma3) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vfma_avx2_f32(a, b, c);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vfma_avx2_f64(a, b, c);
            }
        }
#endif
        ...
        if constexpr (Width == 512 && std::is_floating_point_v<Lane>) {
            if (features_.avx512f) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vfma_avx512_f32(a, b, c);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vfma_avx512_f64(a, b, c);
            }
        }
#endif
        ...
        if constexpr (Width == 128 && std::is_floating_point_v<Lane>) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vfma_neon_f32(a, b, c);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vfma_neon_f64(a, b, c);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vmin(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmin_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmin_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmin_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmin_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmin_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 512) {
            if (features_.avx512f && features_.avx512bw) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmin_avx512_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmin_avx512_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmin_avx512_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vmin_avx512_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmin_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmin_avx512_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmin_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmin_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmin_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmin_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmin_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vmax(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmax_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmax_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmax_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmax_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmax_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 512) {
            if (features_.avx512f && features_.avx512bw) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmax_avx512_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmax_avx512_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmax_avx512_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vmax_avx512_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmax_avx512_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmax_avx512_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vmax_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vmax_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vmax_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vmax_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vmax_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vcmpeq(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vcmpeq_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vcmpeq_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vcmpeq_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vcmpeq_avx2_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vcmpeq_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vcmpeq_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vcmpeq_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vcmpeq_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vcmpeq_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vcmpeq_neon_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vcmpeq_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vcmpeq_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vcmplt(const Vector<Width, Lane>& a,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vcmplt_avx2_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vcmplt_avx2_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vcmplt_avx2_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vcmplt_avx2_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vcmplt_avx2_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vcmplt_avx2_f64(a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int8_t>)
                    return detail::vcmplt_neon_i8(a, b);
                if constexpr (std::is_same_v<Lane, std::int16_t>)
                    return detail::vcmplt_neon_i16(a, b);
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vcmplt_neon_i32(a, b);
                if constexpr (std::is_same_v<Lane, std::int64_t>)
                    return detail::vcmplt_neon_i64(a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vcmplt_neon_f32(a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vcmplt_neon_f64(a, b);
            }
        }
#endif
    ...
    [[nodiscard]] Vector<Width, Lane> dispatch_vblend(const Vector<Width, Lane>& mask,
        ...
        if constexpr (Width == 256) {
            if (features_.avx2) {
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vblend_avx2_i32(mask, a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vblend_avx2_f32(mask, a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vblend_avx2_f64(mask, a, b);
            }
        }
#endif
        ...
        if constexpr (Width == 128) {
            if (features_.neon) {
                if constexpr (std::is_same_v<Lane, std::int32_t>)
                    return detail::vblend_neon_i32(mask, a, b);
                if constexpr (std::is_same_v<Lane, float>)
                    return detail::vblend_neon_f32(mask, a, b);
                if constexpr (std::is_same_v<Lane, double>)
                    return detail::vblend_neon_f64(mask, a, b);
            }
        }
#endif
